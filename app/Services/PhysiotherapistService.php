...
class PhysiotherapistService
{
    ...
    public function getSchedules(array $filters = [], int $perPage = 10): LengthAwarePaginator
    {
        ...
        // When Time of Session Arrived And Physiotherapist Not Started Yet, Change Status to Cancelled
        $now = now();
    ...
    public function formatScheduleData(HomeVisit $visit): array
    {
        ...
        $careProvider = $visit->careProvider;
        return [
            'session_id' => $visit->id,
    ...
    public function acceptOrder(int $id): HomeVisit
    {
        ...
        if ($visit->status == 'accepted') {
            throw new \Exception('This session is already accepted from another physiotherapist', 400);
        }
